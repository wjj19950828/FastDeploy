...
# 转换模型中需要过滤掉的参数
needFilterAttributes = [
    'op_callstack', 'col', 'op_role', 'op_namescope', 'op_role_var',
    'data_format', 'is_test', 'use_mkldnn', 'use_cudnn', 'use_quantizer',
    'workspace_size_MB', 'mkldnn_data_type', 'op_device',
...
def validateShape(shape, name):
    ...
    if len(shape) > 4:
        newShape = shape[-4:]
        print('\033[31m ' + name +
              ' tensor shape length > 4, 处理为丢弃头部shape \033[0m')
        return newShape
    return shape
...
def sliceDataToBinaryFile(paramValueList, outputDir):
    ...
    while currentParamDataIndex < totalParamValuesCount - 1:
        ...
        chunkPath = os.path.join(outputDir, 'chunk_%s.dat' %
                                 (currentChunkIndex + 1))
        file = open(chunkPath, 'wb')
        for i in paramValueList[currentParamDataIndex:currentParamDataIndex +
                                countPerSlice]:
        ...
        print("Output No." + str(currentChunkIndex) + " binary file, remain " +
              str(totalParamValuesCount -
                  currentParamDataIndex) + " param values.")
    print("Slicing data to binary files successfully. (" + str(
        currentChunkIndex) + " output files and " + str(currentParamDataIndex)
          + " param values)")
...
def organizeModelVariableInfo(result):
    ...
    feedData = {}
    feeded_vars = [
        program.global_block().vars[varname] for varname in feed_target_names
    ]
    for feedItem in feeded_vars:
        curShape = feedItem.shape
        feedName = feedItem.name
        feedData[feedName] = np.full(
            excludeNegativeShape(curShape), 1.0, "float32")

    for v in program.list_vars():
        if not v.persistable:
            v.persistable = True
    exe.run(
        program, feed=feedData, fetch_list=fetch_targets, return_numpy=False)
...
def organizeModelOpInfo():
    ...
    for op in ops:
        ...
        if 'X' in inputs and 'Out' in outputs:
            ...
            if len(curInputs) == 1 and len(curOutputs) == 1 and curInputs[
                    0] == curOutputs[0] and index > 1:
                originName = curInputs[0]
                changedName = inputs['X'][0] = curInputs[
                    0] = originName + '_changed'
                ...
                for name in prevOpOutputs:
                    ...
                    for i, curName in enumerate(values):
                        if (curName == originName):
                            modelInfo["ops"][index - 1]['outputs'][name][
                                i] = changedName
                appendedVarList.append({
                    'name': originName,
                    'new': changedName
                })
...
def genModelFeedShape(feed):
    ...
    if len(originFeedShape) == 3:
        ...
        print("\033[33;1mFeedShape length is " + str(len(originFeedShape)) +
              ".\033[0m")
        return
...
def convertToPaddleJSModel(modelDir, modelName, paramsName, outputDir,
    ...
    result = fluid.io.load_inference_model(
        dirname=modelDir,
        executor=exe,
    ...
    try:
        ...
        if not modelDir:
            ...
            if paramDir != modelDir:
                print(
                    "\033[31mModel and param file should be put in a same directory!\033[0m"
                )
                raise Exception()
